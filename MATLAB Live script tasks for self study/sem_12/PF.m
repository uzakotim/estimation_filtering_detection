...
particleMean = scatter(mean(particles),plotHeight(ones(size(mean(particles)))));
delete(particleMean)
pause
...
for t = 1:60
    ...
    delete(particleMean);
    particleHandle = scatter(particles,plotHeight(ones(size(particles))),m*(w+k),'k','filled');
    particleMean = scatter(mean(particles),plotHeight(ones(size(mean(particles)))),'r','filled');
    pause
